...
void main()
{
	...
	// vec3 dist = distance(vec3(gl_ModelViewMatrix * gl_LightSource[0].position), position);
	vec3 dist = distance(gl_LightSource[0].position.xyz, position); // ???
	...
	float thetaI = acos(dot(incidence, normal));
	float thetaR = acos(dot(reflection, normal));
	float phiI = length(incidence);
	float phiR = length(reflection);
	...
	float luminance = lambert * (A + (B * max(0.0, cos(phiI - phiR)) * sin(alpha) * tan(beta)));
